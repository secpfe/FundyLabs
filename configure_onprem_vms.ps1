...
foreach ($account in $ServiceAccounts) {
    ...
    Write-Host "User $account created in ServiceAccounts OU"
    ...
    Write-Host "SPN $account/$serverName assigned"
}
...
foreach ($group in $SecurityGroups + $AdditionalGroups) {
    ...
    Write-Host "Group $group created in Groups OU"
}
...
foreach ($user in $UserAccounts) {
    ...
    New-ADUser -Name $user -SamAccountName $samAccountName -UserPrincipalName "$samAccountName@corp.local" `
    ...
    Write-Host "User $user created with job title $randomTitle in Users OU"
}

# Create specific accounts in Admins OU
New-ADUser -Name $CandiceKevin.Name -SamAccountName $CandiceKevin.SamAccountName -UserPrincipalName "$($CandiceKevin.SamAccountName)@corp.local" `
...
Write-Host "User $($CandiceKevin.Name) created in Admins OU"
Add-ADGroupMember -Identity 'HR' -Members $CandiceKevin.SamAccountName | Out-Null
Write-Host "User $($CandiceKevin.Name) added to group HR"

New-ADUser -Name $JonSmith.Name -SamAccountName $JonSmith.SamAccountName -UserPrincipalName "$($JonSmith.SamAccountName)@corp.local" `
    -AccountPassword (ConvertTo-SecureString $adminPassword -AsPlainText -Force) -Enabled $true `
    -Path "OU=Admins,OU=Corp,$domainDN" | Out-Null
Write-Host "User $($JonSmith.Name) created in Admins OU"
Add-ADGroupMember -Identity 'ITSupport' -Members $JonSmith.SamAccountName | Out-Null
Write-Host "User $($JonSmith.Name) added to group ITSupport"

New-ADUser -Name $BatchAccount.Name -SamAccountName $BatchAccount.SamAccountName -UserPrincipalName "$($BatchAccount.SamAccountName)@corp.local" `
    -AccountPassword (ConvertTo-SecureString $adminPassword -AsPlainText -Force) -Enabled $true `
    -Path "OU=Admins,OU=Corp,$domainDN" | Out-Null
Write-Host "User $($BatchAccount.Name) created in Admins OU"
Add-ADGroupMember -Identity 'Domain Admins' -Members $BatchAccount.SamAccountName | Out-Null
Write-Host "User $($BatchAccount.Name) added to group Domain Admins"
...
foreach ($computer in $ComputerAccounts) {
    ...
    Write-Host "Computer $computer created in Servers OU"
}

Write-Host "Active Directory environment setup with OU structure completed successfully."
